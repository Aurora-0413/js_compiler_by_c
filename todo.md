...
- [ ] **Task 1.1**: 在 parser.y 中实现 ASI 错误恢复机制

  - 在错误恢复规则中检查 `g_lexer.has_newline`
  - 当条件满足时插入虚拟分号并恢复解析
  - 实现三种触发条件的判断逻辑
...
- [ ] **Task 1.2**: 实现受限产生式规则
  ...
  return_statement
      : RETURN ';'                    // 显式分号
      | RETURN expr ';'               // 显式分号
      | RETURN newline                // ASI：return 后遇到换行
          { /* 检查 has_newline，插入分号 */ }
      ;
  ```

  - return 语句的 ASI 处理
  - break/continue 的 ASI 处理
  - throw 语句的 ASI 处理

- [ ] **Task 1.3**: 测试用例开发

  - 创建 `tests/test_asi_basic.js` - 基本 ASI 场景
  - 创建 `tests/test_asi_return.js` - return 受限产生式
  - 创建 `tests/test_asi_complex.js` - 复杂嵌套场景
  - 验证 `a\n++b` 解析为 `a; ++b` 而非 `a++; b`
